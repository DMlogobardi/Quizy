    ...
    public void onBindViewHolder(@NonNull DomandaViewHolder holder, int position) {
        ...
        holder.editTextQuesito.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                domanda.setQuesito(input);

                // Controllo immediato per il feedback visivo
                if (input.trim().length() < 5) {
                    holder.editTextQuesito.setError("Minimo 5 caratteri");
                } else if (input.trim().length() > 20) {
                    holder.editTextQuesito.setError("Massimo 20 caratteri");
                } else {
                    holder.editTextQuesito.setError(null);
                }
            }
