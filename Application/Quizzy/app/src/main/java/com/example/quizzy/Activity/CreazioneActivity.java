...
public class CreazioneActivity extends AppCompatActivity {
    ...
    private void salvaQuiz() {
        ...
        for (int i = 0; i < domandeList.size(); i++) {
            domandaDTO d = domandeList.get(i);

            if (titolo.length() < 5 || titolo.length() > 20) {
                editTextTitolo.setError("Il titolo del quiz deve avere tra 5 e 20 caratteri");
                Toast.makeText(this, "Titolo non valido (5-20 caratteri)", Toast.LENGTH_LONG).show();
                return; // BLOCCA l'invio al server
            }

            if (d.getPuntiRispostaSbagliata() > 0) {
                Toast.makeText(this, "La risposta sbagliata deve essere negativa", Toast.LENGTH_LONG).show();
